...
# Título
doc.add_heading('Instruções para Execução do DAG de ETL com Apache Airflow', 0)

# Introdução
doc.add_paragraph(
    "Este documento fornece as instruções para a configuração e execução de um DAG de ETL "
    "utilizando o Apache Airflow. O DAG extrai dados de arquivos, os transforma e os carrega em um formato de sua escolha."
)

# Código do DAG
doc.add_heading('Código do DAG:', level=1)
...
# Adicionando o código ao documento
doc.add_paragraph(dag_code)

# Instruções para execução
doc.add_heading('Instruções para Execução:', level=1)

instructions = '''1. Configuração do ambiente:
   - Certifique-se de ter o Apache Airflow instalado. Você pode instalar via pip:
     pip install apache-airflow

2. Estrutura de arquivos:
   - O arquivo .json e .parquet precisam estar no caminho "/opt/airflow/dags/database/". Caso esteja utilizando um ambiente diferente, altere os caminhos dos arquivos na função extract() para o diretório correspondente.

3. Executar o Airflow:
   - Inicialize o Airflow no seu terminal:
     airflow db init  # Inicializa o banco de dados do Airflow
     airflow webserver --port 8080  # Inicia o servidor web do Airflow
     airflow scheduler  # Inicia o agendador

4. Subir a DAG:
   - Coloque o arquivo com o código acima na pasta "dags" do seu diretório do Airflow.
   - Acesse o Airflow pelo navegador, normalmente no http://localhost:8080/, e veja a DAG na interface gráfica.

5. Executar a DAG:
   - Na interface do Airflow, você pode acionar a execução manualmente clicando em "Trigger DAG" para rodar a DAG de ETL.
'''

# Adicionando as instruções ao documento
doc.add_paragraph(instructions)


# Salvando o arquivo
file_path = "C:/Users/wesle/OneDrive/Documentos/Teste_Analista_BI/scripts/relatorio_etl_airflow2.docx"
doc.save(file_path)
